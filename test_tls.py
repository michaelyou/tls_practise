...
def open_tls(context, address, server=False):
    # accept和connect结束之后才调用wrap_socket，tls协商发生在tcp三次握手之后
...
def describe(ssl_sock, hostname, server=False, debug=False):
    ...
    if cert is None:
        ...
        say('Name(s) on peer certificate', *names or ['name'])
        ...
        for category, count in sorted(context.cert_store_stats().items()):
            say('Certficates loaded of type {}'.format(category), count)
...
def fill(text):
    return textwrap.fill(text, subsequent_indent='   ', break_long_words=False,
